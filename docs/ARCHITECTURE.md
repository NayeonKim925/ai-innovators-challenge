...
- `POST /api/incidents/{incident_id}/detect` — [ADR-0005](decisions/ADR-0005-agent-first-fault-detection.md) / [AGENT_FAULT_DETECTION_PLAN.md](AGENT_FAULT_DETECTION_PLAN.md). 사람이 cutoff를 고르지 않고, `observed_up_to_s`(재생 시뮬레이터의 현재 위치)까지 관측된 신호만으로 `workflows/detection.py`의 탐지 에이전트가 fault onset을 스스로 추정한다. 두 개의 독립적인 결정론적 신호를 본다 — 알람 최초 활성화(`analytics/fault_onset.py`, Phase 1)와, `real_op` 170개 정상 운전 기록으로 학습한 PCA 재구성오차(`analytics/causrca_anomaly.py`, Phase 2). 두 신호를 조합해 `decision`을 4가지 중 하나로 정한다: `trigger_rca`(알람 활성 + PCA도 corroborate하거나 PCA 미가용 — 그 시각을 cutoff로 `run_investigation`을 자동 실행해 `investigation`/`investigation_id`까지 응답에 포함), `false_positive_review`(알람은 활성인데 PCA는 정상 범위 — 사람에게 오탐 검토 요청, 자동 실행 안 함), `elevated_watch`(알람은 없는데 PCA만 임계치 초과 — 관찰 지속), `await_more_data`(둘 다 조용함). 뒤의 세 경우는 `investigation`이 `null`이다. `data/evaluation`의 `cause_start_at`은 이 경로 어디에서도 읽지 않는다 — 탐지 정확도는 `evals/run_fault_onset_benchmark.py`(라벨 대비 onset lag)와 `evals/run_false_positive_benchmark.py`(`real_op` 170개 대비 오탐률)에서만 채점한다.
