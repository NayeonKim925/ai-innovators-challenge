...
### UI 구성 수정 · Inspo / 2026-09-17

- 사용자 피드백에 따라 네이비·청록은 유지하고 둥근 카드 대시보드에서 조사 기록부형 콘솔로 바꿨다. Inspo 세 레퍼런스와 Lazyweb Databricks/Mixpanel을 실제 확인했으며 [선택 근거](../.lazyweb/design-improve/cluephase-console-2026-09-17/report.md)에 적용·제외 범위를 남겼다.
- 요약은 의미론적 `dl`, 사건은 구분선·선택 레일이 있는 행, 후보·근거는 평면 영역으로 바꿨다. 제목의 두께를 낮추고 사건 ID는 고정폭으로 구분했다. 로컬 폰트를 유지하며 새 의존성은 추가하지 않았다.
- 분석/LLM/API/저장 계약은 변경하지 않았다. 모바일 320px 내비게이션 넘침을 발견해 브랜드와 메뉴 이름을 보존하면서 줄바꿈하도록 수정했다.
- 최종 build/typecheck, Node 테스트 10개, 실제 로컬 API E2E 4개 통과. 조사·근거·검토·질문·보고서·복원·실패복구와 320/390/900/1440px 가로 넘침 검사를 포함한다. 사용자 사용성 조사나 WCAG 인증은 아니다.
- 모바일 결과 화면도 실제 두 번째 사건으로 검증했다. [모바일 분석 결과](ui/mobile-results.png). 기존 메뉴 이름과 브랜드를 숨기지 않는다.
- 배포 이미지: `sha256:eb95324d85dbcbe25faad72e8f254308bc997391362b758b26a83bf36a961053`. 2026-09-17 재개 후 ECS Express 상태 `SUCCESSFUL`, production traffic `100%`를 확인했다.
- 공개 smoke 확인: HTTP 200, 실제 조사 POST 200, 후보 1개, 근거 1개, `llm_status=not_requested`, 브라우저 오류 0개. `dl` 조사 요약과 workspace gap 0px assertion도 통과했다. 공개 검증은 LLM 호출 없이 결정론적 경로로 수행했다.
