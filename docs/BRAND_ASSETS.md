...
- 로고: `frontend/public/brand-mark.svg`를 기본 mark로 사용하고, 기존 `brand-mark.png`는 호환용으로 남긴다. 연속된 C형 궤적과 양 끝의 상태 노드로 “조사가 다음 담당자에게 이어진다”는 의미를 표현한다. 특정 회사의 로고를 복제하지 않았으며 최종 상표 등록 가능성을 검토했다는 의미는 아니다.
